    ...
    if (
        ...
        if columns:
            include_columns = (
                columns
                if has_header
                else [f"f{int(column[7:]) - 1}" for column in columns]
            )
        ...
        with _prepare_file_arg(
            ...
            tbl = pa.csv.read_csv(
                ...
                pa.csv.ParseOptions(
                    delimiter=sep,
                    quote_char=quote_char or False,
...
def read_json(file: str | Path | IOBase, json_lines: bool | None = None) -> DataFrame:
    ...
    return read_ndjson(file) if json_lines else DataFrame._read_json(file)
